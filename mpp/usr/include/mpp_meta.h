...
#define FOURCC_META(a, b, c, d) ((ES_U32)(a) << 24  | \
                                ((ES_U32)(b) << 16) | \
                                ((ES_U32)(c) << 8)  | \
                                ((ES_U32)(d) << 0))
...
typedef enum MppMetaKey_e {
     ...
     * Input data is a MppBufferPtr which contains an array of 16bit Vepu541RoiCfg.
    ...
    KEY_FRAME_NUMBER            = FOURCC_META('f', 'r', 'm', 'n'),
} MppMetaKey;
...
MPP_RET mpp_meta_get_with_tag(MppMetaPtr *meta, const char *tag, const char *caller);
MPP_RET mpp_meta_put(MppMetaPtr meta);
ES_S32  mpp_meta_size(MppMetaPtr meta);

MPP_RET mpp_meta_set_s32(MppMetaPtr meta, MppMetaKey key, ES_S32 val);
MPP_RET mpp_meta_set_s64(MppMetaPtr meta, MppMetaKey key, ES_S64 val);
MPP_RET mpp_meta_set_ptr(MppMetaPtr meta, MppMetaKey key, void  *val);
MPP_RET mpp_meta_get_s32(MppMetaPtr meta, MppMetaKey key, ES_S32 *val);
MPP_RET mpp_meta_get_s64(MppMetaPtr meta, MppMetaKey key, ES_S64 *val);
MPP_RET mpp_meta_get_ptr(MppMetaPtr meta, MppMetaKey key, void  **val);

MPP_RET mpp_meta_set_frame (MppMetaPtr meta, MppMetaKey key, MppFramePtr  frame);
MPP_RET mpp_meta_set_packet(MppMetaPtr meta, MppMetaKey key, MppPacketPtr packet);
MPP_RET mpp_meta_set_buffer(MppMetaPtr meta, MppMetaKey key, MppBufferPtr buffer);
MPP_RET mpp_meta_get_frame (MppMetaPtr meta, MppMetaKey key, MppFramePtr  *frame);
MPP_RET mpp_meta_get_packet(MppMetaPtr meta, MppMetaKey key, MppPacketPtr *packet);
MPP_RET mpp_meta_get_buffer(MppMetaPtr meta, MppMetaKey key, MppBufferPtr *buffer);

MPP_RET mpp_meta_get_s32_d(MppMetaPtr meta, MppMetaKey key, ES_S32 *val, ES_S32 def);
MPP_RET mpp_meta_get_s64_d(MppMetaPtr meta, MppMetaKey key, ES_S64 *val, ES_S64 def);
MPP_RET mpp_meta_get_ptr_d(MppMetaPtr meta, MppMetaKey key, void  **val, void *def);
MPP_RET mpp_meta_get_frame_d(MppMetaPtr meta, MppMetaKey key, MppFramePtr *frame, MppFramePtr def);
MPP_RET mpp_meta_get_packet_d(MppMetaPtr meta, MppMetaKey key, MppPacketPtr *packet, MppPacketPtr def);
MPP_RET mpp_meta_get_buffer_d(MppMetaPtr meta, MppMetaKey key, MppBufferPtr *buffer, MppBufferPtr def);
