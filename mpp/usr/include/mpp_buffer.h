...
/*
 * MppBufferPtr module has several functions:
 *
 * 1. buffer get / put / reference management / external commit / get info.
 *    this part is the basic user interface for MppBufferPtr.
 *
 *    function:
 *
 *    mpp_buffer_get
 ...
 * ion      : use ion device under Android/Linux, MppBufferPtr will encapsulte ion file handle
...
typedef enum {
    ...
    MPP_BUFFER_TYPE_ION,  // reserved
    MPP_BUFFER_TYPE_DRM,  // reserved
 ...
 * hnd  - dma private handle
 * fd   - dma buffer file handle for map / unmap
 *
 */
...
/*
 * mpp_buffer_import_with_tag(MppBufferGroupPtr group, MppBufferInfo *info, MppBufferPtr *buffer)
 *
 * 1. group - specified the MppBufferPtr to be attached to.
 *    group can be NULL then this buffer will attached to default legecy group
 *    Default to NULL on mpp_buffer_import case
 *
 * 2. info  - input information for the output MppBufferPtr
 *    info can NOT be NULL. It must contain at least one of ptr/fd.
 *
 * 3. buffer - generated MppBufferPtr from MppBufferInfo.
 ...
 *
 * Transfer a external buffer info to MppBufferPtr but it is not expected to attached to certain
 ...
 *
 */
MPP_RET mpp_buffer_import_with_tag(MppBufferGroupPtr group, MppBufferInfo *info, MppBufferPtr *buffer,
                                   const char *tag, const char *caller);
MPP_RET mpp_buffer_get_with_tag(MppBufferGroupPtr group, MppBufferPtr *buffer, size_t size,
                                const char *tag, const char *caller);
MPP_RET mpp_buffer_put_with_caller(MppBufferPtr buffer, const char *caller);
MPP_RET mpp_buffer_inc_ref_with_caller(MppBufferPtr buffer, const char *caller);

MPP_RET mpp_buffer_info_get_with_caller(MppBufferPtr buffer, MppBufferInfo *info, const char *caller);
MPP_RET mpp_buffer_read_with_caller(MppBufferPtr buffer, size_t offset, void *data, size_t size, const char *caller);
MPP_RET mpp_buffer_write_with_caller(MppBufferPtr buffer, size_t offset, void *data, size_t size, const char *caller);
void   *mpp_buffer_get_ptr_with_caller(MppBufferPtr buffer, const char *caller);
int     mpp_buffer_get_fd_with_caller(MppBufferPtr buffer, const char *caller);
size_t  mpp_buffer_get_size_with_caller(MppBufferPtr buffer, const char *caller);
int     mpp_buffer_get_index_with_caller(MppBufferPtr buffer, const char *caller);
MPP_RET mpp_buffer_set_index_with_caller(MppBufferPtr buffer, int index, const char *caller);
size_t  mpp_buffer_get_offset_with_caller(MppBufferPtr buffer, const char *caller);
MPP_RET mpp_buffer_set_offset_with_caller(MppBufferPtr buffer, size_t offset, const char *caller);

/**
 * @brief MppBufferPtr cache operation function
 * @param buffer The MppBufferPtr to run the cache operation
...
MPP_RET mpp_buffer_group_get(MppBufferGroupPtr *group, MppBufferType type, MppBufferMode mode,
                             const char *tag, const char *caller);
MPP_RET mpp_buffer_group_put(MppBufferGroupPtr group);
MPP_RET mpp_buffer_group_clear(MppBufferGroupPtr group);
ES_S32  mpp_buffer_group_unused(MppBufferGroupPtr group);
size_t  mpp_buffer_group_usage(MppBufferGroupPtr group);
MppBufferMode mpp_buffer_group_mode(MppBufferGroupPtr group);
MppBufferType mpp_buffer_group_type(MppBufferGroupPtr group);

/*
 * size  : 0 - no limit, other - max buffer size
 * count : 0 - no limit, other - max buffer count
 */
MPP_RET mpp_buffer_group_limit_config(MppBufferGroupPtr group, size_t size, ES_S32 count);

ES_U32 mpp_buffer_total_now();
ES_U32 mpp_buffer_total_max();
